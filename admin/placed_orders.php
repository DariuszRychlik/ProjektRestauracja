...
<h1 class="title">zamówienia</h1>
   ...
   <div class="box">
      ...
      <p> produkty : <span><?= $fetch_orders['total_products']; ?></span> </p>
      <p> razem : <span><?= $fetch_orders['total_price']; ?> PLN</span> </p>
      <p> płatność : <span><?= $fetch_orders['method']; ?></span> </p>
      <form action="" method="POST">
         <input type="hidden" name="order_id" value="<?= $fetch_orders['id']; ?>">
         <select name="payment_status" class="drop-down">
            <option value="" selected disabled><?= $fetch_orders['payment_status']; ?></option>
            <option value="przygotowywanie">przygotowywanie</option>
            <option value="doręczanie">doręczanie</option>
            <option value="gotowe">gotowe</option>
         </select>
         <div class="flex-btn">
            <input type="submit" value="zaktualizuj" class="btn" name="update_payment">
            <a href="placed_orders.php?delete=<?= $fetch_orders['id']; ?>" class="delete-btn" onclick="return confirm('usunąć zamówienie?');">usuń</a>
         </div>
      </form>
   </div>
   <?php
